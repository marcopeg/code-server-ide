...
while [ "$#" -ne 0 ] ; do
    case "$1" in
        ...
        --ec2-key)
            ...
            ;;
        --pg-pwd)
            STACK_PARAMS="${STACK_PARAMS} ParameterKey=QueueDbPassword,ParameterValue=${2}"
            shift 2
            ;;
        --no-sync)
            MODE_SYNC=false
...
CMD="${CMD} cloudformation delete-stack"
...
if [ $MODE_DEBUG = true ] ; then
    ...
    printf "[$(date '+%FT%T%z')] %s\n" "Initiating stack delete for the ${STACK_NAME} [${ENV}] stack"
    $CMD
fi
