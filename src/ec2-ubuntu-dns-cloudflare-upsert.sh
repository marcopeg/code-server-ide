...
then
  ...
  then
    ...
    echo "[$(date -u)] Updating entry ${DNS_ID}: ${CLOUDFLARE_DNS_NAME} -> ${CLOUDFLARE_DNS_TARGET}..." >> ${CODE_SERVER_LOGS}/cloudflare.log
    UPDATE_QUERY=$(
    ...
    echo "[$(date -u)] $(echo $UPDATE_QUERY | jq '.success')" >> ${CODE_SERVER_LOGS}/cloudflare.log
  else
    echo "[$(date -u)] Creating entry: ${CLOUDFLARE_DNS_NAME} -> ${CLOUDFLARE_DNS_TARGET}..." >> ${CODE_SERVER_LOGS}/cloudflare.log
    CREATE_QUERY=$(
    ...
    echo "[$(date -u)] $(echo $CREATE_QUERY | jq '.success')" >> ${CODE_SERVER_LOGS}/cloudflare.log
  fi
  ...
  then
    DNS_ID=$(echo $DNS_QUERY | jq -r '.result[0].id')
    echo "[$(date -u)] Updating entry ${DNS_ID}: ${CLOUDFLARE_DNS_NAME_WILD} -> ${CLOUDFLARE_DNS_TARGET}..." >> ${CODE_SERVER_LOGS}/cloudflare.log
    UPDATE_QUERY=$(
      curl -s -X PUT "https://api.cloudflare.com/client/v4/zones/${CLOUDFLARE_ZONE_ID}/dns_records/${DNS_ID}" \
        -H "Authorization: Bearer ${CLOUDFLARE_API_KEY}" \
        -H "Content-Type: application/json" \
    ...
    echo "[$(date -u)] Creating entry: ${CLOUDFLARE_DNS_NAME_WILD} -> ${CLOUDFLARE_DNS_TARGET}..." >> ${CODE_SERVER_LOGS}/cloudflare.log
    CREATE_QUERY=$(
    curl -s -X POST "https://api.cloudflare.com/client/v4/zones/${CLOUDFLARE_ZONE_ID}/dns_records" \
      -H "Authorization: Bearer ${CLOUDFLARE_API_KEY}" \
      -H "Content-Type: application/json" \
